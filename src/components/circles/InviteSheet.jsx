...
export default function InviteSheet({ open, onOpenChange, circle }) {
  const { user } = useAuth();
  ...
  const handleSendEmail = async () => {
    ...
    setSending(true);
    try {
      await base44.entities.Invitation.update(invitation.id, { invite_email: email.trim() });
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="rounded-t-3xl px-5 pb-8 pt-4 max-h-[85vh] overflow-y-auto">
        ...
        {creating ? (
          ...
          <p className="text-center text-sm text-muted-foreground py-8">Failed to create invitation</p>
        )}
      </SheetContent>
    </Sheet>
  );
